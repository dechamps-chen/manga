    ...
    <Group>
      <BackgroundImage w="100%" h={{ base: height * 0.7, md: 322 }} src={manga.image ? manga.image : ""} style={{ position: "relative" }} >
        ...
        <Box w="100%" h="100%" px={20} style={{ position: "absolute", color: "white", minWidth: 400, alignContent: "flex-end", userSelect: "none" }}>
          <Group w={{ base: "100%", md: 1200 }} mx="auto" pb={20} justify="space-between" align="flex-end">
            <Stack w={{ base: "60%", md: 690 }} pb={20}>
              <Group>
              ...
              <Title order={2} style={{ cursor: "text" }}>{String(manga.title ? manga.title : (manga.altTitles ? (manga.altTitles.find(title => title.en !== undefined)).en : "undefined"))} ({String(manga.releaseDate)})</Title>
              {
